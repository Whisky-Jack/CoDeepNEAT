...
from src.Learner.Layers import MergeSum, MergeCat, DebugWrapper
...
class ModuleNode(Node):
    ...
    def createLayers(self, inFeatures=None, outFeatures=512, debug=False):
        ...
        if self.deepLayer is None:
            ...
            self.deepLayer = nn.Conv2d(self.inFeatures, self.outFeatures, 3, 1, padding=False)
            if debug:
                self.deepLayer.weight.data.fill_(0.2341)
                self.deepLayer.bias.data.fill_(0.341)

            self.combined = [self.deepLayer, self.activation]  # [self.deepLayer, self.activation, nn.MaxPool2d(2, 2)]
